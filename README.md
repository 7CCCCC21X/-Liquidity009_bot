...
| `/booklog` | 📖 日记总览 — 所有在记录的市场 + 一键给**全部**设大单提醒（`/booklog alert 500` 同效）；日记走**独立快轮询**（默认 1 秒，`/booklog speed 1s` 或总览按钮可调，`BOOKLOG_POLL_INTERVAL_MS` 设默认值；普通订阅仍按 `POLL_INTERVAL_MS` 30 秒） |
